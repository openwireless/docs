...
MGIM(V4.1)は、4GM(V1)と互換性のある下記の機能を提供します（ただし、機能の使い方は大きく異なります）
...
|対応SIMカード|microSIM|[利用実績のあるSIMカード](docs/sim_usable_mgim.md)|
...
下表に、HL7800向けArduinoライブラリ **hl7800** が提供する機能の一覧を示します。
...
| センサ | 加速度センサ割り込み設定 | 加速度センサの割り込みハンドラを設定／解除する | 加速度センサはlis2dwライブラリを使って制御 |

### 補足

  - 各ライブラリの使い方は、各ライブラリのサンプルスケッチを参照してみてください。
  - UDPとTCP機能では、各々一つのコネクションまで利用できます（つまり、同時に利用できます）
  - GPS関連の機能は、今後、上記ライブラリに組み込んでいく予定です。
